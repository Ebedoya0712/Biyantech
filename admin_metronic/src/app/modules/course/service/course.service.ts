...
export class CourseService {
  ...
  // 🚨 NUEVO MÉTODO PARA LISTAR PAGOS MÓVILES PENDIENTES
  listPagosMovilPendientes(search: any, status: any, page: number = 1) {
    this.isLoadingSubject.next(true);
    let headers = new HttpHeaders({ 'Authorization': 'Bearer ' + this.authservice.token });

    let URL = URL_SERVICIOS + `/pagos/movil-pendientes?page=${page}`;

    // (Opcional: Añadir parámetros de búsqueda y estado si los necesitaras en el futuro)
  ...
  // 🚨 NUEVO MÉTODO PARA APROBAR PAGOS MÓVILES
  approvePagoMovil(saleId: number) {
    this.isLoadingSubject.next(true);
    let headers = new HttpHeaders({ 'Authorization': 'Bearer ' + this.authservice.token });
    let URL = URL_SERVICIOS + `/pagos/movil-aprobar/${saleId}`;
  ...
  // 🚨 MÉTODOS PARA TRAILER DE NUEVO CURSO
  getTrailer() {
    this.isLoadingSubject.next(true);
    let headers = new HttpHeaders({ 'Authorization': 'Bearer ' + this.authservice.token });
    let URL = URL_SERVICIOS + "/course-trailer";
    return this.http.get(URL, { headers: headers }).pipe(
      finalize(() => this.isLoadingSubject.next(false))
    );
  }

  saveTrailer(data: any) {
    this.isLoadingSubject.next(true);
    let headers = new HttpHeaders({ 'Authorization': 'Bearer ' + this.authservice.token });
    let URL = URL_SERVICIOS + "/course-trailer";
    return this.http.post(URL, data, { headers: headers }).pipe(
  ...
  uploadVideoTrailer(data: any) {
    this.isLoadingSubject.next(true);
    let headers = new HttpHeaders({ 'Authorization': 'Bearer ' + this.authservice.token });
    let URL = URL_SERVICIOS + "/course-trailer/upload_video";
    return this.http.post(URL, data, { headers: headers }).pipe(
      finalize(() => this.isLoadingSubject.next(false))
    );
  }
